            ...
            <form action="{{ route('pengeluaran.update', $pengeluaran->id) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <select name="tahun" id="tahun" class="form-control @error('tahun') is-invalid @enderror" required>
                        ...
                        @foreach($tahunList as $tahun)
                            <option value="{{ $tahun }}" {{ old('tahun', $pengeluaran->tahun) == $tahun ? 'selected' : '' }}>
                                {{ $tahun }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('nama_pengeluaran') is-invalid @enderror" 
                           ...
                           value="{{ old('nama_pengeluaran', $pengeluaran->nama_pengeluaran) }}" required>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('keterangan') is-invalid @enderror" 
                              id="keterangan" name="keterangan" rows="3">{{ old('keterangan', $pengeluaran->keterangan) }}</textarea>
                    @error('keterangan')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <small class="text-muted">Masukkan nominal (contoh: 1.000.000)</small>
                    @error('nominal')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
